...
self.addEventListener('notificationclick', (event) => {
  ...
  async function tryAck(notifId) {
    ...
    try {
      ...
      await tellPages('ACK_RESULT', { ok, status: res.status, body, id: notifId });
      return { ok, status: res.status };
    } catch (err) {
      await tellPages('ACK_RESULT', { ok: false, status: -1, error: String(err), id: notifId });
